...
export const setting1 = (deviceId:number,type:number) => {
    return request({
        url: path + '/setting/' + deviceId + '/' + type,
        method: 'post'
    });
}

export const online1 = (type:string) => {
    return request({
        url: path + '/online/' + type,
        method: 'post'
    });
}
...
export const heartbeat = (deviceId:string ) => {

    const formData = new FormData();
    formData.append("deviceId",deviceId)
    return request({
        url:  '/api/device/heartbeat/list',
        method: 'post',
        data: formData
    });
}
